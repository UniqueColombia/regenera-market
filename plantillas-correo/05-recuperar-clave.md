...
**Plantilla de Supabase:** *Reset Password*

> ## ⚠️ Hoy la aplicación NO usa esta plantilla
>
> «Olvidé mi contraseña» en Seregenera **es** entrar con código y definir una
> nueva: mismo correo, misma plantilla (*Magic Link*), un camino menos que se
> pueda romper sin que nadie se entere. Está razonado en la cabecera de
> `src/app/entrar/actions.ts`.
>
> Se deja escrita por si algún día alguien llama a `resetPasswordForEmail()`:
> ese día, la plantilla de fábrica llega en inglés y solo con
> `{{ .ConfirmationURL }}` — o sea, sin código y con un enlace que no pasa por
> `/auth/callback`. Pegar esto la deja lista de antemano.
>
> **Configurarla no la activa.** Solo se manda si alguien llama a esa función.

## Asunto

```
Recupera el acceso a Seregenera
```
...
<head>
  ...
  <title>Recupera el acceso a Seregenera</title>
</head>
<body style="margin:0; padding:0; background-color:#f0ebe2;">

  <div style="display:none; max-height:0; overflow:hidden; opacity:0; mso-hide:all;">
    Tu código para recuperar el acceso: {{ .Token }}
  </div>
          ...
          <tr>
            <td style="padding:12px 32px 8px 32px;">
              ...
              <p style="margin:14px 0 0 0; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; font-size:16px; line-height:1.6; color:#55655c;">
                Pediste recuperar el acceso a tu cuenta de Seregenera. Escribe
                este código y podrás elegir una contraseña nueva:
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 32px 0 32px;">
              ...
              <p style="margin:20px 0 0 0; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; font-size:13px; line-height:1.6; color:#55655c;">
                Si no pediste recuperar nada, ignora este correo: tu contraseña
                actual sigue funcionando y no cambia hasta que alguien use este
                código, que vence solo. Nadie de Seregenera te lo va a pedir por
                teléfono, por WhatsApp ni por correo.
              </p>
            </td>
          </tr>
